...
- The board page can now show the same tasks as a sortable list. A Board / List toggle sits beside the page title, which names the view you are in; there is no second page and no new entry in the sidebar. Both views read the same working set, so the search box, the employee, competency, project and priority filters, "Active only", and the count pills all apply unchanged when you switch. The list has a row per task with Task, Assignee, Project, Column, Due, Priority, and Status; clicking a heading sorts by it and clicking it again reverses the order. Tasks with no due date, no assignee, or no project stay at the bottom in both directions rather than crowding the top. Priority sorts by rank rather than alphabetically, and sorting by Column — the default — reproduces the board's own order, so switching views never rearranges work that was arranged by hand. A task's column can be changed from its row, which applies the same status rules and performance-input prompt as dragging its card between lanes. Hand-ordering within a column remains the board's job. The view resets to Board when RADAR reopens.
