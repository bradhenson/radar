...
- Refined the shared light/dark surfaces, typography, controls, tables, and navigation. The original Work, People, and System navigation order is preserved; existing accent palettes and glass appearances remain available.
...
- Deleting default board columns (such as Complete) now persists across application restarts. Default columns are only seeded on initial database creation, preserving any column removals made by the user.
...
- Glass is now the Look RADAR opens with. New databases start there, and an existing one still on Standard — the previous default, so a setting nobody chose — moves across once. Any database already set to Glass or Glass+ is untouched, and Standard can be picked again in Settings → Appearance; that choice is a deliberate one and is never revisited. Reduced-transparency and high-contrast system settings still fall back to solid panels as before.

- The rich-text editor now also covers project descriptions, award supporting notes, and check-in summaries, with the same formatting, toolbar, and keyboard access as everywhere else. A project description keeps its paragraphs and lists in the expanded project row, which previously ran the whole thing together as a single block. Text already in these three fields converts once and exactly as typed: they have always been plain boxes, so a line beginning "- " or "#" stays those characters instead of silently becoming a list or a heading. The conversion is offered on the same backup-first prompt as the earlier one, and backups exported before this change convert on import (backup format version 6).

- Every rich-text field in RADAR now uses a maintained editing engine (Tiptap) instead of RADAR's own contenteditable code: notes, quick notes, task descriptions and task notes, meeting discussion notes and action items, performance context/action/result, and employee notes. Typing, selection, undo/redo, and input-method composition are handled by the editor rather than by hand-written repair logic. Formatting is unchanged apart from checklists: headings, bold, italic, underline, bulleted lists, and numbered lists all behave as before, and the toolbar keeps its keyboard access (Alt+F10, arrow keys, Escape back to the text). Checkboxes are no longer part of rich text anywhere — a task's dedicated checklist is unaffected. Existing content is read and converted automatically, and any checklist inside older text becomes a bulleted list whose items keep a ✓ or ○ marker, so no item and no completion state is lost. Search, CSV export, and task-to-performance imports read the converted text, and the optional MCP server now writes the same format. The single-file artifact grows from roughly 598 KB to 967 KB as a result; it still opens from `file://` with no server and makes no network requests.
...
- Web addresses in rich text are now links. Typing or pasting a full address turns it into one as you go, and a Link button in the editor toolbar attaches an address to whatever text is selected — so "the telework policy" can carry the link instead of the address sitting in the middle of a sentence. The same button edits or removes the link under the cursor. Links open in your browser in a new tab; in the desktop app they open in your default browser rather than inside the RADAR window. Where text is displayed rather than edited — a note, a meeting note, a check-in — a link is an ordinary click. Where it is only ever seen in an editor, a task description most of all, **Ctrl+click** opens it, the same gesture Word uses; a plain click still just places the cursor. Holding Ctrl turns the pointer into a hand over a link, so the gesture shows itself the way it does in Word, and the Link button's tooltip names it too. With the cursor inside a link that button also offers Open, so the link can be followed without a mouse. Addresses are limited to web and email links, and anything else is refused when it is typed and again when it is displayed, so a note from an imported backup cannot turn a click into something else. An address that fails that check still shows its text — the words are never lost, only the link. Notes saved before this change are unaffected and are read as they always were.

- The board page can now show the same tasks as a sortable list. A Board / List toggle sits beside the page title, which names the view you are in; there is no second page and no new entry in the sidebar. Both views read the same working set, so the search box, employee, competency, project and priority filters, and count pills all apply unchanged when you switch. The list has a row per task with Task, Assignee, Project, Column, Due, Priority, Status, and the same Done action as a board card; clicking a heading sorts by it and clicking it again reverses the order. Tasks with no due date, no assignee, or no project stay at the bottom in both directions rather than crowding the top. Priority sorts by rank rather than alphabetically, and sorting by Column — the default — reproduces the board's own order, so switching views never rearranges work that was arranged by hand. A task's column can be changed from its row, which applies the same Open/Waiting status rules as dragging its card between lanes. Hand-ordering within a column remains the board's job. The view resets to Board when RADAR reopens.

- The employee overview has a Check-ins tab listing every check-in recorded for that person, newest first, with its date, interaction type, a marker where follow-up was flagged, and the summary itself. The date and the edit control both open the check-in in the same dialog used to record one, which can now correct an existing entry's date, type, summary, and follow-up flag. The roster's last check-in date only ever moves forward, so backdating or correcting an entry cannot hide a more recent one.
...
- Employee profiles gained three built-in fields under Assets and requirements: Gov Passport and SIPR Account (Yes / No / In process) and Passport Expiration (date). They behave like every other profile field — editable on the profile, searchable, exportable, and available to the MCP server by label — and existing databases pick them up automatically without losing any profile customization.

- Telework requests are now tracked against the bi-weekly pay period allowance. The situational request list shows everything from the last 30 days plus all upcoming requests (denied and cancelled ones included — they are part of the record of who asked for what), grouped by pay period with the current period marked. Each row carries that employee's running use for its period — "2 of 2", red when over — and the group header names how many people are over. Pending requests count toward the tally so a third request is visible before it is approved; denied and cancelled ones read "Not counted". Requests spanning a period boundary split their days across both periods, and weekends inside a range are not counted as telework days. Settings gained a Telework and pay periods card: days allowed per pay period (default 2), the pay period start date the bi-weekly cycle counts from (defaulting to the federal calendar, where the July 2026 periods begin on the 12th and the 26th), and how far back the list reaches. The card shows the resulting current pay period so the anchor can be confirmed at a glance. The CSV export gained telework days, pay period, days used, and allowance columns.

- Travel now tracks the whole trip lifecycle. Vouchers can be marked submitted (or reopened) from an expanded row, and trips that fall through can be cancelled instead of deleted — the record is kept, the trip stops counting as an absence on the calendar and availability report, and no voucher is expected. The list groups rows by where each trip stands — Voucher due, On travel now, Upcoming, then Completed and Cancelled — with quick-filter pills above it showing the counts. A returned trip stays on the list until its voucher is settled, so nobody quietly drops off the list still owing paperwork; completed and cancelled trips appear only under "Show past travel". Cancelling, marking, and reopening a voucher each write an activity entry and offer Undo, and the CSV export gained trip status, voucher status, and voucher submitted columns.
